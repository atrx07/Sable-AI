    ...
    def read_file_lines(self, path: str, start: int = 1, end: int | None = None) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                "read_file_lines",
                True,
                output=f"Lines {start}-{end} of {len(lines)}:\n{selected}",
                truncated=truncated,
            )
        except OSError as exc:
        ...
        try:
            for root, dirs, files in os.walk(target):
                dirs[:] = [
                    ...
                    if d not in {".git", ".sable", "node_modules", "__pycache__", "venv", ".venv"}
                ]
                root_path = Path(root)
        ...
        try:
            ...
            for raw in glob.glob(str(target / "**" / pattern), recursive=True):
                ...
                try:
                    ...
                    continue
                if is_blocked_path(rel) or any(
                    part in {".git", ".sable", "node_modules", "venv", ".venv"} for part in p.parts
                ):
                    continue
                matches.append(rel)
                if len(matches) >= 200:
        ...
        try:
            for p in target.rglob("*"):
                ...
                if not p.is_file() or any(
                    part in {".git", ".sable", "node_modules", "venv", ".venv", "__pycache__"}
                    for part in p.parts
                ):
                    continue
    ...
    def file_info(self, path: str) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                "file_info",
                True,
                output=f"{kind}: {self._rel(target)}\nsize: {stat.st_size} bytes\nmode: {oct(stat.st_mode & 0o777)}",
            )
        except OSError as exc:
            ...
            if target.is_file():
                ...
                for p in target.rglob("*"):
                    if p.is_file() and not any(
                        part in {".git", "node_modules", "venv", ".venv"} for part in p.parts
                    ):
                        try:
                            ...
                            pass
            return ToolResult(
                "disk_usage", True, output=f"{self._rel(target)}: {total / (1024 * 1024):.2f} MiB"
            )
        except OSError as exc:
