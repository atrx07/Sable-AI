...
def _rate(
    results: list[EvalResult], predicate: Callable[[EvalResult], bool]
) -> dict[str, float | int]:
...
def aggregate_metrics(results: Iterable[EvalResult]) -> dict:
    items = list(results)
    completed = [
        item for item in items if item.disposition in {EvalDisposition.PASS, EvalDisposition.FAIL}
    ]
    ...
    refusal = [
        ...
        in {
            ...
            ExpectedOutcome.CAPABILITY_DENIED,
        }
    ]
    security = [
        item
    ...
    rollback = [
        ...
        if item.expected_outcome
        in {
            ExpectedOutcome.ROLLBACK_SUCCESS,
    ...
    return {
        ...
        "total_tokens": _distribution(
            item.token_usage.get("total_tokens", 0) for item in completed
        ),
        ...
        "skipped_count": sum(
            1
            for item in items
            if item.disposition not in {EvalDisposition.PASS, EvalDisposition.FAIL}
        ),
    }
