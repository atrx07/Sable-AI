...
class TaskTransaction:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "TaskTransaction":
        ...
        values["checkpoints"] = [
            TransactionCheckpoint.from_dict(item) for item in data.get("checkpoints", [])
        ]
        return cls(**values)
...
class WorkspaceTransactionManager:
    ...
    def __init__(
        ...
        *,
        ...
        self.storage_root = Path(
            storage_dir or (CONFIG_DIR / "transactions" / workspace_key)
        ).expanduser()
    ...
    def _prepare_storage(self) -> None:
        try:
            ...
            self.storage_root = (
                Path(tempfile.gettempdir()) / "sable-transactions" / self._workspace_key
            )
            self.storage_root.mkdir(parents=True, exist_ok=True)
        try:
        ...
        for metadata in self.storage_root.glob("*/metadata.json"):
            try:
                ...
                if tx.status == TransactionStatus.OPEN.value:
                    ...
                    tx.rollback_status = (
                        RollbackStatus.AVAILABLE.value
                        if tx.snapshots
                        else RollbackStatus.UNAVAILABLE.value
                    )
                    self._save(tx)
                loaded.append(tx)
    ...
    def begin(self, label: str = "task") -> str:
        ...
        self.current = TaskTransaction(
            ...
            git_head=git["head"],
            branch=git["branch"],
            baseline_staged=git["staged"],
            baseline_unstaged=git["unstaged"],
    ...
    def _git_paths(self, args: list[str], repo_root: Path) -> list[str]:
        ...
        for raw in output.split("\0"):
            ...
            try:
                paths.append(
                    (repo_root / raw).resolve(strict=False).relative_to(self.root).as_posix()
                )
            except ValueError:
                continue
    ...
    def _git_baseline(self) -> dict[str, Any]:
        ...
        return {
            ...
            "staged": self._git_paths(
                ["diff", "--cached", "--name-only", "-z", "--no-ext-diff"], repo_root
            ),
    ...
    def _is_conflict_sensitive(self, rel: str) -> bool:
        if self.current is None:
            return False
        dirty = (
            self.current.baseline_staged
            + self.current.baseline_unstaged
            + self.current.baseline_untracked
        )
    ...
    def _target(self, relative_path: str) -> Path:
        ...
        if (
            ...
            or ".." in pure.parts
            or not pure.parts
            or is_blocked_path(relative_path)
        ):
        ...
        try:
            ...
            raise TransactionError(
                f"Transaction parent escapes workspace: {relative_path}"
            ) from exc
        return target
    ...
    def _inspect_tree(self, target: Path) -> tuple[int, int]:
        ...
        if target.is_file():
            ...
            if size > self.max_file_backup_bytes:
                raise TransactionError(
                    f"File snapshot exceeds {self.max_file_backup_bytes // (1024 * 1024)} MiB: {target.name}"
                )
            return size, 1
        ...
        for base, dirs, files in os.walk(target, followlinks=False):
            ...
            for name in list(dirs) + list(files):
                ...
                if is_blocked_path(rel):
                    raise TransactionError(
                        f"Refused to snapshot protected path inside mutation target: {rel}"
                    )
                if path.is_symlink():
                    try:
                        ...
                        raise TransactionError(
                            f"Snapshot symlink escapes workspace: {rel}"
                        ) from exc
                    if is_blocked_path(resolved_rel):
                        raise TransactionError(
                            f"Snapshot symlink resolves to protected path: {rel}"
                        )
                elif path.is_file():
                    size = path.stat().st_size
                    if size > self.max_file_backup_bytes:
                        raise TransactionError(
                            f"File snapshot exceeds {self.max_file_backup_bytes // (1024 * 1024)} MiB: {rel}"
                        )
                    total += size
                    if total > self.max_backup_bytes:
                        raise TransactionError(
                            f"Transaction snapshot exceeds {self.max_backup_bytes // (1024 * 1024)} MiB."
                        )
    ...
    def _fingerprint(target: Path) -> str:
        ...
        if target.is_symlink():
            digest.update(
                b"symlink\0" + os.readlink(target).encode("utf-8", errors="surrogateescape")
            )
        ...
        for base, dirs, files in os.walk(target, followlinks=False):
            ...
            for name in dirs + files:
                path = base_path / name
                digest.update(
                    path.relative_to(target).as_posix().encode("utf-8", errors="surrogateescape")
                    + b"\0"
                )
                if path.is_symlink():
                    digest.update(
                        b"link\0" + os.readlink(path).encode("utf-8", errors="surrogateescape")
                    )
                elif path.is_file():
                    digest.update(b"file\0")
    ...
    def _snapshot_path(self, target: Path, rel: str, backup_name: str) -> PathSnapshot:
        ...
        if self.current and self.current.backup_bytes + size > self.max_backup_bytes:
            raise TransactionError(
                f"Refused transactional mutation: backup budget would exceed {self.max_backup_bytes // (1024 * 1024)} MiB."
            )
        slot = (
            self.current.directory / backup_name
            if self.current
            else self.storage_root / backup_name
        )
        ...
        if target.is_symlink():
            ...
            snapshot.link_target = os.readlink(target)
        elif target.is_dir():
            snapshot.kind = "directory"
            shutil.copytree(target, slot, symlinks=True)
        else:
            snapshot.kind = "file"
            shutil.copy2(target, slot, follow_symlinks=False)
        return snapshot
    ...
    def capture(self, target: str | Path) -> None:
        ...
        if any(
            s.relative_path == rel or rel.startswith(s.relative_path.rstrip("/") + "/")
            for s in self.current.snapshots
        ):
            return
    ...
    def checkpoint(self, label: str = "checkpoint") -> str | None:
        ...
        for index, (baseline, target) in enumerate(zip(self.current.snapshots, targets)):
            snapshot = self._snapshot_path(
                target, baseline.relative_path, f"checkpoints/{checkpoint_id}/{index:04d}"
            )
    ...
    def _rollback(self, transaction: TaskTransaction, *, dry_run: bool = False) -> dict[str, Any]:
        ...
        transaction.status = (
            ...
            if complete
            else TransactionStatus.PARTIAL_ROLLBACK.value
        )
        transaction.rollback_status = (
            RollbackStatus.ROLLED_BACK.value if complete else RollbackStatus.PARTIAL.value
        )
    ...
    def restore_checkpoint(self, checkpoint_id: str | None = None) -> dict[str, Any]:
        ...
        checkpoint = next(
            (
                item
                for item in reversed(self.current.checkpoints)
                if checkpoint_id is None or item.checkpoint_id == checkpoint_id
            ),
            None,
        )
        ...
        if plan["conflicts"]:
            raise TransactionError(
                "Checkpoint restore refused because files changed outside Sable: "
                + ", ".join(plan["conflicts"][:8])
            )
    ...
    def finish(
        ...
        *,
        ...
        actual_changes = (
            transaction.created_files + transaction.modified_files + transaction.deleted_files
        )
        transaction.rollback_status = (
            RollbackStatus.AVAILABLE.value
            if transaction.snapshots and actual_changes
            else RollbackStatus.UNAVAILABLE.value
        )
        self._save(transaction)
        self.current = None
    ...
    def _latest_eligible(self) -> TaskTransaction | None:
        for transaction in reversed(self.history):
            if transaction.rollback_status in {
                RollbackStatus.AVAILABLE.value,
                RollbackStatus.PARTIAL.value,
            }:
                return transaction
        return None
    ...
    def undo(self, transaction_id: str | None = None, *, dry_run: bool = False) -> dict[str, Any]:
        ...
        if transaction.rollback_status not in {
            RollbackStatus.AVAILABLE.value,
            RollbackStatus.PARTIAL.value,
        }:
            raise TransactionError(
                f"Transaction {transaction.transaction_id} is not rollback-eligible."
            )
    ...
    def _public(transaction: TaskTransaction, detail: bool = False) -> dict[str, Any]:
        changed = list(
            dict.fromkeys(
                transaction.created_files + transaction.modified_files + transaction.deleted_files
                or transaction.touched_files
            )
        )
        data: dict[str, Any] = {
            "transaction_id": transaction.transaction_id,
            "task_summary": transaction.task_summary,
            "status": transaction.status,
        ...
        if detail:
            data.update(
                {
                    "created_files": list(transaction.created_files),
                    "modified_files": list(transaction.modified_files),
                    "deleted_files": list(transaction.deleted_files),
                    "git_head": transaction.git_head,
                    "branch": transaction.branch,
                    "baseline_staged": list(transaction.baseline_staged),
                    "baseline_unstaged": list(transaction.baseline_unstaged),
                    "baseline_untracked": list(transaction.baseline_untracked),
                    "checkpoints": [
                        {
    ...
    def _save(self, transaction: TaskTransaction) -> None:
        ...
        fd, tmp_name = tempfile.mkstemp(
            prefix=".metadata-", suffix=".json", dir=str(transaction.directory), text=True
        )
        tmp = Path(tmp_name)
        try:
    ...
    def close(self) -> None:
        if self.current is not None:
            ...
            self.current.rollback_status = (
                RollbackStatus.AVAILABLE.value
                if self.current.snapshots
                else RollbackStatus.UNAVAILABLE.value
            )
            self._save(self.current)
