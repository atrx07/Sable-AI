...
class ApprovalEngine:
    ...
    def decide(self, request_id: str, decision: ApprovalDecision | str) -> CapabilityRequest:
        ...
        if request is None or request.decision is not None or request.session_id != self.session_id:
            raise ApprovalError(
                "Approval request is invalid, stale, already consumed, or belongs to another session."
            )
        try:
            normalized = (
                decision
                if isinstance(decision, ApprovalDecision)
                else ApprovalDecision(str(decision).upper())
            )
    ...
    def authorize(self, request: CapabilityRequest) -> AuthorizationOutcome:
        ...
        if request.session_id != self.session_id:
            return AuthorizationOutcome(
                False, request.capability, "Approval session changed.", "stale", request=request
            )
        if grant_key in self._session_grants:
            ...
            return AuthorizationOutcome(
                True,
                request.capability,
                "Matching session-scoped approval reused.",
                "session",
                request=request,
            )
        ...
        try:
            ...
            self._pending.pop(request.request_id, None)
            return AuthorizationOutcome(
                False, request.capability, redact_secrets(str(exc))[:300], "denied", request=request
            )
        ...
        return AuthorizationOutcome(
            False, request.capability, "Human approval denied.", "denied", request=decided
        )
...
class CapabilityPolicy:
    ...
    def evaluate(
        self, capability: Capability, *, mode: str, source: ActionSource
    ) -> AuthorizationOutcome:
        normalized_mode = mode if mode in {"plan", "build", "yolo"} else "build"
        if capability in self.HARD_DENIED:
            return AuthorizationOutcome(
                False, capability, "External filesystem access is a hard boundary.", "hard_boundary"
            )
        if normalized_mode == "plan" and capability != Capability.READ_WORKSPACE:
            return AuthorizationOutcome(
                False, capability, "Plan mode is a hard read-only ceiling.", "plan_ceiling"
            )
        ...
        if capability in self.BASELINE:
            return AuthorizationOutcome(
                True, capability, "Allowed by the mode baseline.", "baseline"
            )
        ...
        return AuthorizationOutcome(
            False, capability, "Human approval is required.", "approval", approval_required=True
        )
...
def _command_requirements(argv: list[str]) -> list[CapabilityRequirement]:
    ...
    package_install = executable in PACKAGE_MANAGERS and any(
        item in PACKAGE_ACTIONS for item in lowered
    )
    if executable.startswith("python") and "-m" in lowered:
        try:
            package_install = package_install or lowered[lowered.index("-m") + 1] in {
                "pip",
                "ensurepip",
            }
        except IndexError:
            pass
    git_remote = executable == "git" and any(
        item in {"clone", "fetch", "pull", "push"} for item in lowered
    )
    ...
    if executable == "git" and "push" in lowered:
        requirements.append(
            CapabilityRequirement(
                Capability.GIT_PUBLISH, action, "high", "The command publishes Git refs."
            )
        )
    elif git_remote:
        requirements.append(
            CapabilityRequirement(
                Capability.GIT_REMOTE_READ, action, "high", "The command accesses a Git remote."
            )
        )
    return requirements
...
def requirements_for_tool(tool: str, args: dict[str, Any]) -> list[CapabilityRequirement]:
    ...
    if name in READ_TOOLS or (name == "git_branch" and not args.get("name")):
        return [
            CapabilityRequirement(
                ...
                name,
                "normal",
                "Read workspace or local Git metadata.",
                target,
            )
        ]
    if name in WRITE_TOOLS:
        return [
            CapabilityRequirement(
                ...
                name,
                "normal",
                "Mutate a workspace path through a transaction-aware file tool.",
                target,
            )
        ]
    if name == "delete_file":
        return [
            CapabilityRequirement(
                ...
                f"delete {target or '?'}",
                "high",
                "Delete a workspace path through the transaction-aware file tool.",
                target,
            )
        ]
    if name == "run_command":
    ...
    if name == "run_shell":
        ...
        return [
            CapabilityRequirement(
                ...
                command,
                "high",
                "Execute a raw shell command whose effects cannot be fully classified.",
            )
        ]
    if name in {"git_clone", "git_fetch", "git_pull"}:
        ...
        return [
            CapabilityRequirement(
                Capability.NETWORK_ACCESS, action, "high", "Access a Git remote."
            ),
            CapabilityRequirement(
                Capability.GIT_REMOTE_READ, action, "high", "Read or merge data from a Git remote."
            ),
        ]
    if name == "git_push":
        ...
        return [
            ...
            CapabilityRequirement(
                Capability.GIT_PUBLISH, action, "high", "Publish local Git refs to a remote."
            ),
        ]
    if name in {"git_commit", "git_stash", "git_set_remote"} or (
        name == "git_branch" and args.get("name")
    ):
        return [
            CapabilityRequirement(
                Capability.GIT_HISTORY_MUTATION,
                name,
                "high",
                "Mutate local Git state or configuration.",
                target,
            )
        ]
    return []
...
def approval_scope_material(
    tool: str, args: dict[str, Any], capability: Capability
) -> dict[str, Any]:
