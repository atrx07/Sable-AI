...
class FailureClassifier:
    ...
    @staticmethod
    def classify(
        check: VerificationCheck, status: CheckStatus, result: ToolResult, diagnostic: str
        ...
        if re.search(
            r"no module named|cannot find module|could not resolve dependenc|package .* not found|dependency .* missing",
            text,
        ):
        ...
        if re.search(
            r"out of memory|memoryerror|no space left|resource temporarily unavailable", text
        ):
            return FailureClassification.RESOURCE_LIMIT
        if check.category == CheckCategory.TYPECHECK or re.search(
            r"type error|incompatible type|is not assignable to", text
        ):
    ...
    @staticmethod
    def signature(
        check: VerificationCheck, classification: FailureClassification, diagnostic: str
    ) -> str:
        ...
        normalized = re.sub(
            r"\b\d+(?:\.\d+)?\s*(?:ms|s|sec|seconds|minutes)\b", "<duration>", normalized
        )
    ...
    def analyze(
        self, check: VerificationCheck, status: CheckStatus, result: ToolResult
