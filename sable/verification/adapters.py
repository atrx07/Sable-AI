    ...
    def executable(self, executable: str, cwd: str = ".") -> tuple[CheckAvailability, str]:
        ...
        if candidate.is_absolute() or "/" in executable or "\\" in executable:
            ...
            if os.name == "nt" and not local.suffix:
                variants.extend(
                    (
                        local.with_suffix(".cmd"),
                        local.with_suffix(".bat"),
                        local.with_suffix(".exe"),
                    )
                )
            for path in variants:
                ...
                if resolved.is_file():
                    return (
                        ...
                        "Local project executable exists inside the workspace.",
                    )
        ...
        return (
            ...
            f"Configured executable is unavailable on PATH: {executable}",
        )
...
class VerificationAdapter(ABC):
    ...
    def affected(self, changed_files: Iterable[str]) -> tuple[str, ...]:
        ...
        return tuple(
            ...
            if not prefix or path == self.cwd or path.startswith(prefix)
        )[:100]
        ...
        try:
            ...
            for current, dirnames, filenames in os.walk(self.project_root):
                ...
                dirnames[:] = [
                    ...
                    if name not in {".git", ".venv", "venv", "node_modules", "build", "dist"}
                    and depth < 5
                ]
    ...
    def discover(self, changed_files: tuple[str, ...]) -> list[VerificationCheck]:
        ...
        if pyproject and (pyproject.count("[") != pyproject.count("]") or pyproject.count('"') % 2):
            self.warnings.append(
                f"Malformed TOML may limit discovery: {self._display('pyproject.toml')}"
            )
        ...
        lower = (pyproject + "\n" + setup_cfg + "\n" + tox_ini).lower()
        pytest_configured = (
            self.exists("pytest.ini")
            or "[tool.pytest" in lower
            or "[tool:pytest]" in lower
            or "pytest" in tox_ini.lower()
        )
        ...
        configured = (
            (
                ...
                "ruff",
                ["ruff", "check", "."],
                self.exists("ruff.toml") or self.exists(".ruff.toml") or "[tool.ruff" in lower,
            ),
            (
                "Flake8",
                CheckCategory.LINT,
                "flake8",
                ["flake8", "."],
                self.exists(".flake8") or "[flake8]" in lower,
            ),
            (
                "Black check",
            ...
            (
                ...
                "mypy",
                ["mypy", "."],
                self.exists("mypy.ini") or "[tool.mypy" in lower or "[mypy]" in lower,
            ),
            (
                "Pyright",
                CheckCategory.TYPECHECK,
                "pyright",
                ["pyright"],
                self.exists("pyrightconfig.json") or "[tool.pyright" in lower,
            ),
        )
            ...
            if enabled:
                checks.append(
                    self.check(
                        ...
                        changed_files,
                        scope=VerificationScope.QUICK
                        if category in {CheckCategory.LINT, CheckCategory.FORMAT}
                        else VerificationScope.AFFECTED,
                        toolchain=tool,
                        required=False,
...
class NodeAdapter(VerificationAdapter):
    ...
    def _script_availability(self, manager: str, package: dict) -> tuple[CheckAvailability, str]:
        ...
        if (
            available == CheckAvailability.AVAILABLE
            and dependencies
            and not self.exists("node_modules")
        ):
            return (
                CheckAvailability.UNAVAILABLE,
                "Package dependencies are not installed; verification will not auto-install them.",
            )
    ...
    def discover(self, changed_files: tuple[str, ...]) -> list[VerificationCheck]:
        ...
        if self.exists("package.json"):
            try:
                ...
                self.warnings.append(
                    f"Malformed package.json at {self._display('package.json')}: {exc}"
                )
                package = {}
        ...
        for script, category, scope in known_scripts:
            ...
            if (
                ...
                or not value.strip()
                or "no test specified" in value.lower()
            ):
                continue
        ...
        direct: list[tuple[str, CheckCategory, str, list[str], VerificationScope, bool]] = [
            (
                ...
                self.exists("tsconfig.json") and "typecheck" not in selected_scripts,
            ),
            (
                "ESLint",
                CheckCategory.LINT,
                ...
                "lint" not in selected_scripts
                and any(
                    name.startswith("eslint.config.") or name.startswith(".eslintrc")
            ...
            (
                ...
                ["prettier", "--check", "."],
                VerificationScope.QUICK,
                "format:check" not in selected_scripts
                and any(
                    name.startswith(".prettierrc") or name == "prettier.config.js"
                    for name in config_names
                ),
            ),
            (
                ...
                ["vitest", "run"],
                VerificationScope.AFFECTED,
                "test" not in selected_scripts
                and (
                    "vitest" in dependencies
                    or any(name.startswith("vitest.config.") for name in config_names)
                ),
            ),
            (
                "Jest",
                ...
                and (
                    ...
                    or any(name.startswith("jest.config.") for name in config_names)
                ),
            ),
        ]
    ...
    def discover(self, changed_files: tuple[str, ...]) -> list[VerificationCheck]:
        ...
        checks = [
            self.check(
                ...
                scope=VerificationScope.QUICK,
                availability=cargo,
                reason="Cargo.toml defines a Rust package or workspace.",
                dependencies=("locally cached or vendored crates",),
            ),
            self.check(
                "cargo test",
                CheckCategory.UNIT_TEST,
                ...
                scope=VerificationScope.AFFECTED,
                availability=cargo,
                reason="Cargo.toml defines deterministic Rust tests.",
                dependencies=("locally cached or vendored crates",),
            ),
        ]
        ...
        checks.extend(
            (
                self.check(
                    ...
                    scope=VerificationScope.QUICK,
                    required=False,
                    availability=rustfmt,
                    reason="Rust formatting validation is supported for Cargo projects.",
                ),
                self.check(
                    "cargo clippy",
                    CheckCategory.LINT,
                    ["cargo", "clippy", "--quiet", "--offline", "--", "-D", "warnings"],
                    changed_files,
                    scope=VerificationScope.FULL,
                    required=False,
                    availability=clippy,
                    reason="Clippy static analysis is supported when installed.",
                    dependencies=("locally cached or vendored crates",),
                ),
            )
        )
    ...
    def discover(self, changed_files: tuple[str, ...]) -> list[VerificationCheck]:
        ...
        return [
            self.check(
                ...
                scope=VerificationScope.AFFECTED,
                availability=go,
                reason="A Go module/workspace manifest was detected.",
                dependencies=("declared modules without modifying go.mod/go.sum",),
            ),
            self.check(
                "go vet",
                CheckCategory.STATIC_ANALYSIS,
                ["go", "vet", "-mod=readonly", "./..."],
                changed_files,
                scope=VerificationScope.AFFECTED,
                availability=go,
...
class JavaAdapter(VerificationAdapter):
    ...
    def _wrapper(self, stem: str, system: str) -> tuple[str, tuple[CheckAvailability, str]]:
        variants = (
            [f"{stem}.cmd", f"{stem}.bat", stem]
            if os.name == "nt"
            else [stem, f"{stem}.cmd", f"{stem}.bat"]
        )
            ...
            checks.extend(
                (
                    self.check(
                        "Maven test",
                        CheckCategory.UNIT_TEST,
                        [executable, "--batch-mode", "--offline", "test"],
                        changed_files,
                        scope=VerificationScope.AFFECTED,
                        toolchain="maven",
                        availability=state,
                        reason="pom.xml defines a Maven project; repository wrapper is preferred.",
                        dependencies=("locally cached Maven dependencies",),
                    ),
                    self.check(
                        "Maven verify",
                        CheckCategory.BUILD,
                        [executable, "--batch-mode", "--offline", "verify"],
                        changed_files,
                        scope=VerificationScope.FULL,
                        toolchain="maven",
                        availability=state,
                        reason="pom.xml defines Maven's full verification lifecycle.",
                        dependencies=("locally cached Maven dependencies",),
                    ),
                )
            )
            ...
            checks.extend(
                (
                    self.check(
                        "Gradle test",
                        CheckCategory.UNIT_TEST,
                        [executable, "--offline", "test"],
                        changed_files,
                        scope=VerificationScope.AFFECTED,
                        toolchain="gradle",
                        availability=state,
                        reason="A Gradle build manifest was detected; repository wrapper is preferred.",
                        dependencies=("locally cached Gradle dependencies",),
                    ),
                    self.check(
                        "Gradle check",
                        CheckCategory.BUILD,
                        [executable, "--offline", "check"],
                        changed_files,
                        scope=VerificationScope.FULL,
                        toolchain="gradle",
                        availability=state,
                        reason="Gradle's configured verification lifecycle is available.",
                        dependencies=("locally cached Gradle dependencies",),
                    ),
                )
            )
