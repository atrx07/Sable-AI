...
def diagnose(
    ...
    *,
    ...
    cfg = dict(
        _load_config_read_only(Path(config_file or CONFIG_FILE)) if config is None else config
    )
    ...
    report.checks.append(
        DiagnosticCheck(
            "Sable",
            "python",
            "PASS" if py_ci_tested else ("WARN" if py_minimum else "FAIL"),
            f"Python {sys.version.split()[0]} (CI-tested: 3.10-3.13)",
            critical=not py_minimum,
        )
    )
    ...
    if is_blocked_path(str(root)):
        report.checks.append(
            DiagnosticCheck(
                ...
                "protected workspace",
                "FAIL",
                "refused before repository or verification discovery",
                critical=True,
            )
        )
        return report
    ...
    report.checks.append(
        DiagnosticCheck(
            "Workspace",
            "write access",
            "PASS" if writable else ("FAIL" if build_expected else "WARN"),
    ...
    report.checks.extend(
        [
            ...
            DiagnosticCheck(
                "Provider",
                "main model",
                "PASS" if cfg.get("main_model") else "FAIL",
                _model_detail(cfg.get("main_model")),
                critical=not bool(cfg.get("main_model")),
            ),
            DiagnosticCheck(
                "Provider",
                "fast model",
                "PASS" if cfg.get("fast_model") else "FAIL",
                _model_detail(cfg.get("fast_model")),
                critical=not bool(cfg.get("fast_model")),
            ),
            DiagnosticCheck(
                "Provider",
                "API key",
    ...
    if not git_path:
        report.checks.append(
            DiagnosticCheck("Git", "executable", "UNAVAILABLE", "git not found on PATH")
        )
    else:
        ...
        if not in_repo:
            ...
            report.checks.extend(
                [
                    DiagnosticCheck(
                        "Git", "repository", "PASS", f"repository on {branch or 'detached HEAD'}"
                    ),
                    DiagnosticCheck(
                        "Git",
                        "working tree",
                    ...
                    DiagnosticCheck(
                        "Git",
                        "origin",
                        "PASS" if remote else "INFO",
                        "configured" if remote else "not configured",
                    ),
                ]
            )
    ...
    if requested_backend not in {"auto", "native", "proot"}:
        ...
        selected = (
            native if requested_backend == "native" or not proot_availability.available else proot
        )
    ...
    if requested_backend in {"auto", "native", "proot"}:
        report.checks.append(
            DiagnosticCheck(
                "Execution",
                "backend",
                "PASS" if availability.available else "FAIL",
                f"{selected.name}: {availability.reason}",
    ...
    report.checks.extend(
        [
            DiagnosticCheck(
                "Execution",
                "Termux",
                "PASS" if is_termux_environment() else "INFO",
                "detected" if is_termux_environment() else "not detected",
            ),
            DiagnosticCheck(
                "Execution",
                "PRoot",
                "PASS" if proot_availability.available else "UNAVAILABLE",
    ...
    missing_isolation = [
        name
        for name in ("filesystem_namespace", "network_isolation", "process_isolation")
    ...
    report.checks.append(
        DiagnosticCheck(
            "Execution",
            "isolation",
            "WARN" if missing_isolation else "PASS",
            "not supported: " + ", ".join(missing_isolation)
            if missing_isolation
            else "configured guarantees available",
        )
    )
    ...
    try:
        ...
        report.checks.append(
            DiagnosticCheck(
                "Verification", "scope", "WARN", "invalid configured scope; affected fallback"
            )
        )
    else:
        report.checks.append(
            DiagnosticCheck(
                "Verification",
                "configuration",
                "PASS" if verify_enabled else "WARN",
    ...
    try:
        ...
        report.checks.append(
            DiagnosticCheck(
                "Verification", "toolchains", "PASS" if discovery.adapters else "INFO", adapters
            )
        )
        ...
        for warning in discovery.warnings[:10]:
            report.checks.append(
                DiagnosticCheck("Verification", "discovery", "WARN", redact_secrets(warning)[:500])
            )
    except (OSError, ValueError) as exc:
        report.checks.append(
            DiagnosticCheck("Verification", "discovery", "WARN", redact_secrets(str(exc))[:500])
        )
    ...
    for name in ("session storage", "trace persistence", "transaction storage"):
        report.checks.append(
            DiagnosticCheck(
                ...
                if control_ok
                else ("temporary fallback accessible" if fallback_ok else "no writable storage"),
...
def render_text(report: DoctorReport) -> str:
    ...
    lines.extend(
        [
            ...
            "Summary: "
            + " | ".join(
                f"{counts[name]} {name.lower()}"
                for name in ("PASS", "WARN", "INFO", "UNAVAILABLE", "FAIL")
                if counts[name]
            ),
