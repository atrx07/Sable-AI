...
class AffectedSelectionTests(unittest.TestCase):
    ...
    def test_python_checks_are_refined_to_changed_source_and_affected_test(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            planner.discovery = VerificationDiscovery(
                root, executable_finder=lambda name: f"/bin/{name}"
            )
    ...
    def test_node_direct_runner_uses_proximate_test_but_script_falls_back_safely(self):
        ...
        refined, selected = selector.refine_checks(
            [direct, script], ("src/foo.ts",), context, VerificationScope.AFFECTED
        )
...
class ScopeEscalationTests(unittest.TestCase):
    ...
    def test_ci_workflow_change_escalates_scope(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            self.assertEqual(
                planner.plan([".github/workflows/ci.yml"]).scope, VerificationScope.FULL
            )
    ...
    def test_custom_run_override_is_not_scope_escalated(self):
        with tempfile.TemporaryDirectory() as root:
            planner = VerificationPlanner(root)
            plan = planner.plan(
                ["pyproject.toml"], scope="quick", custom_command="python --version"
            )
...
class FailureClassifierTests(unittest.TestCase):
    ...
    def test_dependency_tool_timeout_policy_and_resource_classification(self):
        self.assertEqual(
            self.classify("ModuleNotFoundError: No module named 'demo'")[0],
            FailureClassification.DEPENDENCY_MISSING,
        )
        self.assertEqual(
            self.classify("missing", status=CheckStatus.SKIPPED_UNAVAILABLE)[0],
            FailureClassification.TOOL_MISSING,
        )
        self.assertEqual(
            self.classify("timed out", status=CheckStatus.TIMEOUT)[0], FailureClassification.TIMEOUT
        )
        self.assertEqual(
            self.classify("denied", status=CheckStatus.BLOCKED)[0],
            FailureClassification.POLICY_BLOCKED,
        )
        self.assertEqual(
            self.classify("MemoryError: out of memory")[0], FailureClassification.RESOURCE_LIMIT
        )

    def test_diagnostic_is_bounded_and_secret_redacted(self):
        secret = "GROQ_API_KEY=gsk_abcdefghijklmnopqrstuvwxyz123456"
        classification, diagnostic, signature = self.classify(
            secret + "\n" + ("failure line\n" * 2000)
        )
